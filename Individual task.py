...
def bubble_sort(array):  # Функція методу сортування бульбашкою
    ...
    for i in range(len(array)):  # Кількість перевірок всього масиву залежить від його довжини
        swapped = False  # swapped - змінна, яка показує чи відбувся обмін елементів місцями
        ...
        for j in range(len(array) - i - 1):
            ...
            if array[j] > array[j + 1] and order == 'a':
                ...
                swapped = True  # Якщо обмін відбувся змінна swapped приймає істинне значення
        ...
        # Якщо під час перевірки масиву не відбулось жодного обміну елементів місцями, то сортування завершується
        if not swapped:
            break
